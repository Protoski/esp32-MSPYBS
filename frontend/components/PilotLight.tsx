...
const config: Record<DeviceStatus, { dot: string; label: string; ring: string }> = {
  ON:    { dot: 'bg-green-400 shadow-[0_0_12px_3px_rgba(74,222,128,0.6)]', ring: 'ring-green-500/30', label: 'OPERATIVO' },
  OFF:   { dot: 'bg-slate-600', ring: 'ring-slate-600/20', label: 'APAGADO' },
  FAULT: { dot: 'bg-red-500 shadow-[0_0_14px_4px_rgba(239,68,68,0.7)] animate-pulse', ring: 'ring-red-500/40', label: 'FALLA' },
};
    ...
    <div className={`flex items-center gap-4 rounded-xl bg-slate-800 border border-slate-700 p-4 ring-2 ${ring}`}>
      ...
      <div className="min-w-0">
        ...
        <p className={`text-xs font-bold tracking-widest uppercase ${
          ...
          status === 'FAULT' ? 'text-red-400 animate-pulse' : 'text-slate-500'
        }`}>{statusLabel}</p>
