    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        {!isLoaded ? (
          <button
            onClick={load}
            disabled={isLoading}
            className="px-4 py-2 bg-sky-600 hover:bg-sky-500 disabled:opacity-50 text-white font-bold text-sm rounded-lg transition-colors"
          >
      ...
      {!isLoaded && !isLoading && (
        <div className="rounded-xl border border-dashed border-slate-700 p-12 text-center">
          ...
          <button onClick={load} className="px-5 py-2.5 bg-sky-600 hover:bg-sky-500 text-white font-bold rounded-lg transition-colors">
            Cargar datos ahora
          </button>
        </div>
      )}
      ...
      {isLoaded && !isLoading && (
        <>
          ...
          <div className="flex flex-wrap gap-3">
            <select
              value={selectedZone}
              onChange={e => { setSelectedZone(e.target.value); setSelectedHospital('all'); }}
            ...
            >
              ...
              {hospList
                .filter(h => selectedZone === 'all' || getZone(h.ciudad) === selectedZone)
...
function ZoneTab({ byZone }: { byZone: Record<string, ZoneSummary> }) {
  ...
  const purityData = {
    ...
    datasets: [{
      label: 'Pureza O₂ promedio (%)',
      data: zones.map(z => +z.avgPurity.toFixed(2)),
      backgroundColor: zones.map(z => `${ZONE_COLORS[z.zona] ?? '#38bdf8'}80`),
      borderColor: zones.map(z => ZONE_COLORS[z.zona] ?? '#38bdf8'),
      borderWidth: 2,
      borderRadius: 6,
    }],
  };
  ...
  const flowData = {
    labels: zones.map(z => z.zona),
    datasets: [{
      label: 'Caudal O₂ promedio (m³/h)',
      data: zones.map(z => +z.avgFlow.toFixed(2)),
  ...
  const onlineData = {
    ...
    datasets: [{
      data: zones.map(z => z.hospitals.length),
      backgroundColor: zones.map(z => `${ZONE_COLORS[z.zona] ?? '#38bdf8'}80`),
      borderColor: zones.map(z => ZONE_COLORS[z.zona] ?? '#38bdf8'),
      borderWidth: 2,
    }],
    ...
    <div className="space-y-4">
            ...
            <div key={zs.zona} className="rounded-xl bg-slate-800/80 border border-slate-700 p-4" style={{ borderLeftColor: color, borderLeftWidth: 3 }}>
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ background: `${color}20`, color }}>
                  {zs.onlineCount}/{zs.hospitals.length} online
                </span>
              </div>
              <div className="grid grid-cols-3 gap-2">
                {[
                  ['Pureza', `${zs.avgPurity.toFixed(1)}%`],
                  ['Caudal', `${zs.avgFlow.toFixed(1)} m³/h`],
                  ['Vacío', `${zs.avgVacuum.toFixed(0)} mmHg`],
                ].map(([l, v]) => (
                  <div key={l}>
                    <p className="text-[9px] text-slate-500 uppercase">{l}</p>
                    <p className="text-xs font-bold mt-0.5" style={{ color }}>{v}</p>
                  </div>
                ))}
              </div>
              <div className="mt-2 flex flex-wrap gap-1">
                {zs.hospitals.map(h => (
                  <span key={h.hospital.id} className="text-[9px] text-slate-500 bg-slate-900/50 px-1.5 py-0.5 rounded">
                    {h.hospital.nombre}
                  </span>
                ))}
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 rounded-xl bg-slate-800/80 border border-slate-700 p-4">
          ...
          <div className="h-56">
            <Bar data={purityData} options={{ ...CHART_DEFAULTS, scales: { ...CHART_DEFAULTS.scales, y: { ...CHART_DEFAULTS.scales.y, min: 88, max: 100 } } }} />
          </div>
        </div>
      ...
      <div className="rounded-xl bg-slate-800/80 border border-slate-700 p-4">
        ...
        <div className="h-48">
          <Bar data={flowData} options={CHART_DEFAULTS} />
        </div>
      </div>
    </div>
  );
...
function TrendTab({ data }: { data: HospitalData[] }) {
  ...
  const metricLabels: Record<string, string> = {
    o2_purity_pct: 'Pureza O₂ (%)',
    o2_flow_m3h: 'Caudal O₂ (m³/h)',
    o2_tank_pressure_bar: 'Presión Tanque O₂ (bar)',
    vacuum_level_mmhg: 'Nivel Vacío (mmHg)',
  };
  ...
  const chartData = {
    labels: (() => {
      const allTs = data.flatMap(d => d.rows.map(r => r.timestamp)).filter(Boolean) as string[];
      const sorted = [...new Set(allTs)].sort();
      return sorted.slice(-50).map(ts => new Date(ts).toLocaleTimeString('es-CO', { hour: '2-digit', minute: '2-digit' }));
    })(),
    datasets: data
      .filter(d => d.rows.length > 0)
      .map((d, i) => {
        const allTs = data.flatMap(dd => dd.rows.map(r => r.timestamp)).filter(Boolean) as string[];
        const sorted = [...new Set(allTs)].sort().slice(-50);
        const tsMap: Record<string, number> = {};
        d.rows.forEach(r => { if (r.timestamp) tsMap[r.timestamp] = r[metric] as number; });
        return {
          label: d.hospital.nombre,
          data: sorted.map(ts => tsMap[ts] ?? null),
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap gap-2">
        {Object.entries(metricLabels).map(([k, l]) => (
          <button
            key={k}
            onClick={() => setMetric(k as typeof metric)}
            className={`px-3 py-1 text-xs font-bold rounded-lg transition-all ${metric === k ? 'bg-sky-600 text-white' : 'bg-slate-800 text-slate-400 hover:text-slate-200 border border-slate-700'}`}
          >
            {l}
          </button>
        ))}
      ...
      <div className="rounded-xl bg-slate-800/80 border border-slate-700 p-4">
        ...
        <div className="h-72">
          <Line data={chartData} options={{ ...CHART_DEFAULTS, plugins: { ...CHART_DEFAULTS.plugins, legend: { ...CHART_DEFAULTS.plugins.legend } } }} />
        </div>
      </div>

      {/* Mini sparklines per hospital */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {data.filter(d => d.rows.length > 0).map((d, i) => {
          ...
          const sparkData = {
            labels: vals.map((_, j) => String(j)),
            datasets: [{ data: vals, borderColor: color, backgroundColor: `${color}20`, borderWidth: 1.5, pointRadius: 0, tension: 0.3, fill: true }],
          };
          return (
            <div key={d.hospital.id} className="rounded-xl bg-slate-800/80 border border-slate-700 p-3">
              <div className="flex justify-between items-start mb-2">
                <div>
                  <p className="text-xs font-bold text-slate-200">{d.hospital.nombre}</p>
                  <p className="text-[9px] text-slate-500">{d.hospital.ciudad}</p>
                </div>
...
function ComparativeTab({ data }: { data: HospitalData[] }) {
  ...
  return (
    <div className="space-y-4">
      {metrics.map(m => {
        const chartData = {
          labels: data.map(d => d.hospital.nombre),
          datasets: [{
            label: m.label,
            ...
            <div className="h-44">
              <Bar
                ...
                options={{
                  ...
                  scales: {
                    x: { ...CHART_DEFAULTS.scales.x, ...(m.min !== undefined ? { min: m.min } : {}) },
...
function ConsumptionTab({ data }: { data: HospitalData[] }) {
  ...
  // Estimate hourly consumption buckets
  ...
  const hourAvgs = hourLabels.map(l => {
    const v = hourBuckets[l] ?? [];
  ...
  const hourlyChart = {
    ...
    datasets: [{
      label: 'Caudal O₂ promedio (m³/h)',
      data: hourAvgs.map(v => +v.toFixed(2)),
      borderColor: '#38bdf8',
  ...
  // Per-zone total flow
  const zoneTotals = data.reduce<Record<string, number>>((acc, d) => {
    const z = d.zona;
    const total = d.rows.reduce((a, r) => a + r.o2_flow_m3h, 0);
    acc[z] = (acc[z] ?? 0) + total;
    return acc;
  }, {});
  ...
  const zoneFlowChart = {
    labels: zoneNames,
    datasets: [{
      label: 'Flujo total acumulado (m³)',
      data: zoneNames.map(z => +zoneTotals[z].toFixed(1)),
      backgroundColor: zoneNames.map(z => `${ZONE_COLORS[z] ?? '#38bdf8'}80`),
      borderColor: zoneNames.map(z => ZONE_COLORS[z] ?? '#38bdf8'),
      borderWidth: 2,
      borderRadius: 6,
    }],
  };

  // Per-hospital stack: last 50 rows timeline
  const timelineTs = [...new Set(data.flatMap(d => d.rows.map(r => r.timestamp)).filter(Boolean) as string[])].sort().slice(-30);
  const stackedChart = {
    labels: timelineTs.map(ts => new Date(ts).toLocaleTimeString('es-CO', { hour: '2-digit', minute: '2-digit' })),
    datasets: data.filter(d => d.rows.length > 0).map((d, i) => {
      const tsMap: Record<string, number> = {};
      d.rows.forEach(r => { if (r.timestamp) tsMap[r.timestamp] = r.o2_flow_m3h; });
      return {
        label: d.hospital.nombre,
        data: timelineTs.map(ts => tsMap[ts] ?? 0),
  ...
  const purityCounts = purityBins.map(bin =>
    allRows.filter(r => r.o2_purity_pct >= bin && r.o2_purity_pct < bin + 1).length
  );
  const purityDist = {
    labels: purityBins.map(b => `${b}–${b+1}%`),
    datasets: [{
      label: 'Frecuencia lecturas',
      data: purityCounts,
      backgroundColor: purityBins.map(b =>
        b >= 95 ? '#34d39980' : b >= 93 ? '#38bdf880' : b >= 90 ? '#fb923c80' : '#ef444480'
      ),
      borderColor: purityBins.map(b =>
        b >= 95 ? '#34d399' : b >= 93 ? '#38bdf8' : b >= 90 ? '#fb923c' : '#ef4444'
      ),
      borderWidth: 1.5,
      borderRadius: 4,
    }],
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="rounded-xl bg-slate-800/80 border border-slate-700 p-4">
          ...
          <div className="h-56">
            <Line data={hourlyChart} options={CHART_DEFAULTS} />
          </div>
        </div>
        <div className="rounded-xl bg-slate-800/80 border border-slate-700 p-4">
          ...
          <div className="h-56">
            <Bar data={zoneFlowChart} options={CHART_DEFAULTS} />
          </div>
        </div>
      </div>
      ...
      <div className="rounded-xl bg-slate-800/80 border border-slate-700 p-4">
        ...
        <div className="h-56">
          <Bar data={stackedChart} options={{ ...CHART_DEFAULTS, scales: { x: { ...CHART_DEFAULTS.scales.x, stacked: true }, y: { ...CHART_DEFAULTS.scales.y, stacked: true } } }} />
        </div>
      </div>
      ...
      <div className="rounded-xl bg-slate-800/80 border border-slate-700 p-4">
        ...
        <div className="h-48">
          <Bar data={purityDist} options={{ ...CHART_DEFAULTS, plugins: { ...CHART_DEFAULTS.plugins, legend: { display: false } } }} />
        </div>
      </div>
    </div>
  );
