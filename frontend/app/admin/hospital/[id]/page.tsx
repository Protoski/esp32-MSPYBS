...
export default function HospitalConfigPage() {
  ...
  if (!form || !hospital) return <div className="flex items-center justify-center h-48 text-slate-500">Cargando configuración…</div>;
    ...
    <div className="max-w-2xl space-y-6">
      <div className="flex items-start justify-between gap-3">
        <div>
          <div className="flex items-center gap-2 text-sm text-slate-500 mb-1">
            ...
            <span>/</span><span className="text-slate-300">{hospital.nombre}</span>
          </div>
          <h1 className="text-xl font-black text-slate-100">Configuración</h1>
        </div>
        <div className="flex gap-2">
          <Link href={`/hospital/${id}`} className="px-3 py-2 rounded-xl border border-slate-700 text-slate-400 hover:text-sky-400 hover:border-sky-500/50 text-xs font-semibold transition-all">Ver dashboard</Link>
          <button onClick={handleToggle} className={`px-3 py-2 rounded-xl text-xs font-bold transition-all border ${
            hospital.activo ? 'bg-red-500/10 text-red-400 hover:bg-red-500/20 border-red-500/30' : 'bg-green-500/10 text-green-400 hover:bg-green-500/20 border-green-500/30'}`}>
            {hospital.activo ? '⏸ Desactivar' : '▶ Activar'}
          </button>
        </div>
      </div>
      {success && <div className="rounded-xl bg-green-500/10 border border-green-500/30 px-4 py-3 text-sm text-green-400 font-semibold">✅ Configuración guardada correctamente.</div>}
      ...
      <div className="rounded-xl border border-slate-700 bg-slate-800/60 overflow-hidden">
        <div className="px-4 py-3 border-b border-slate-700 bg-slate-800 flex items-center justify-between">
          ...
          <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${hospital.activo ? 'bg-green-500/20 text-green-400' : 'bg-slate-700 text-slate-500'}`}>{hospital.activo ? 'Activo' : 'Inactivo'}</span>
        </div>
      ...
      <div className="rounded-xl border border-slate-700 bg-slate-800/60 overflow-hidden">
        <div className="px-4 py-3 border-b border-slate-700 bg-slate-800"><p className="text-sm font-bold text-slate-300">⚠️ Umbrales de Alarma</p></div>
        <div className="p-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
          {[
      ...
      <div className="rounded-xl border border-slate-700 bg-slate-800/60 overflow-hidden">
        ...
        <div className="p-4 space-y-3">
          {[
            ...
            return (
              <div key={key} className={`flex items-center justify-between rounded-xl border px-4 py-3 transition-all ${
                enabled ? 'border-green-500/30 bg-green-500/5' : 'border-slate-700 bg-slate-900/40'}`}>
                <div><p className="text-sm font-semibold text-slate-200">{label}</p><p className="text-[10px] text-slate-500 mt-0.5">{desc}</p></div>
                <button onClick={() => setEq(key, !enabled)}
                  className={`relative w-12 h-6 rounded-full transition-colors flex-shrink-0 ${enabled ? 'bg-green-500' : 'bg-slate-700'}`}>
      ...
      <div className="flex gap-3">
        <button onClick={() => router.back()} className="flex-1 rounded-xl border border-slate-700 text-slate-400 hover:text-slate-200 py-3 text-sm font-semibold transition-all">Cancelar</button>
        <button onClick={handleSave} disabled={saving} className="flex-1 rounded-xl bg-sky-600 hover:bg-sky-500 disabled:opacity-50 text-white py-3 text-sm font-bold transition-colors">{saving ? 'Guardando…' : '💾 Guardar Cambios'}</button>
      </div>
    </div>
  );
}
